...
#define WAV_SIZE      512    // The size of each generated waveform.  The larger the size the higher
...
void generateTriangle(int32_t amplitude, int32_t* buffer, uint16_t length) {
  ...
  for (int i = 0; i < length / 2; i++) {
    buffer[i] = -(amplitude / 2) + (delta * i) + (amplitude / 4);
  }
    for (int i=length/2; i<length; ++i) {
    buffer[i] = (amplitude/2)-delta*(i-length/2) - (amplitude / 4);
  }
}
...
void playWave(int32_t* buffer, uint16_t length, float frequency, float seconds) {
  ...
  i2s_set_clk(I2S_NUM_0, SAMPLERATE_HZ, I2S_BITS_PER_SAMPLE_16BIT, (i2s_channel_t)2);
...
void loop() {
  ...
  for (int i=0; i<sizeof(scale)/sizeof(float); ++i) {
    ...
    playWave(sine, WAV_SIZE, scale[i], 0.01);
  ...
  for (int i=0; i<sizeof(scale)/sizeof(float); ++i) {
    // Play the note for a quarter of a second.
    playWave(sawtooth, WAV_SIZE, scale[i], 0.01);
    // Pause for a tenth of a second between notes.
    delay(100);
  }
  ...
  for (int i=0; i<sizeof(scale)/sizeof(float); ++i) {
    // Play the note for a quarter of a second.
    playWave(triangle, WAV_SIZE, scale[i], 0.01);
    // Pause for a tenth of a second between notes.
    delay(100);
  }
  ...
  for (int i=0; i<sizeof(scale)/sizeof(float); ++i) {
    // Play the note for a quarter of a second.
    playWave(square, WAV_SIZE, scale[i], 0.01);
    // Pause for a tenth of a second between notes.
    delay(100);
  }
}